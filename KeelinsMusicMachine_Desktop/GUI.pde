...
class TextField extends Widget {
 ...
 TextField(String val, int x, int y, int w, int h) {
  super("", x, y, w, h);
  value = val;
 }
 ...
 void display() {
  ...
  fill(0);
  if (on)
   text(value + "-", pos.x + 5, pos.y + extents.y);
  else
   text(value, pos.x + 5, pos.y + extents.y);
 }
}
...
class ComboBox extends Widget {
 TextField[] options;
 boolean open = false, last = false;
 int select;

 ComboBox(int numOptions, int x, int y, int w, int h) {
  super("", x, y, w, h * numOptions);
  options = new TextField[numOptions];
  for (int i = 0; i < options.length; i++) {
   ...
   options[i] = new TextField("", bx, by, w, h);
   if (i == 1) options[i].toggle();
  }
 }
 ...
 boolean mouseReleased() {
  for (int i = 1; i < options.length; i++) {
   ...
   if (options[i].mouseReleased()) {
    reset();
    options[i].open();
    options[0].set(": "+options[i].get());
    return true;
   }
  }
  return false;
